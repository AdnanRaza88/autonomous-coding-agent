...
An active run renders as a chat transcript: the goal as the user turn, each worker as an assistant block that grows on `agent_delta`, and a short status line when the run ends. Chat stays primary; Plan shows the same DAG in topological batches so parallel work is visible. The last surface is kept in `sessionStorage`. The idle screen keeps the empty-state hints. While a run is planning or running the composer stays locked and exposes Stop (Esc). After it settles, Continue starts a new run seeded with the original goal plus shipped drafts. Failed or cancelled runs offer Retry, which restarts from the original goal. Copy writes the thread to the clipboard. New returns to the empty chat. The last provider and model are kept in localStorage.

## Screens

- Run: chat transcript of the live goal, streaming worker drafts, Chat/Plan toggle over the live DAG, Stop in the composer while live, copy/new after settle, Retry on failed or cancelled runs, follow-up from the composer, remembered provider/model pickers
