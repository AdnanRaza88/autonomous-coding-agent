...
const HINTS = [
  ...
  "Add integration tests for the permission gate",
  "Ship a Vercel deploy for the active run",
  "Register a specialist subagent for reviews",
]
          ...
          <div className="mt-8 grid gap-2 text-left sm:grid-cols-2">
            {HINTS.map((text) => (
              <button
                key={text}
                type="button"
                className="rounded-xl border border-line px-3.5 py-3 text-left text-[13px] leading-snug text-muted transition-colors hover:border-ink/20 hover:text-ink"
                onClick={() => props.onHint?.(text)}
              >
                {text}
              </button>
            ))}
